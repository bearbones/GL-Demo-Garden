...
4. **Fracture Modes** — inspired by Sellán et al., *Breaking Good* (TOG 2022): at bake time, Dijkstra finds the K cheapest paths across a seeded toughness grid (overlap-penalized for diversity) — the slab's intrinsic ways to break. Each path gets a deterministic Brownian-bridge midpoint displacement plus Brownian-walk side branches (lightning geometry, seeded so restamps align). Strikes only *reveal* the nearest path outward from the strike (`stamp-segments.glsl` draws capsule segments); re-strikes deepen, widen, and run it further. Pottery-style: a few long crackly lines that pre-exist the first tap. No per-frame simulation at all.

   This model also shatters differently: at the break moment every struck crack *snaps* — its reveal completes through the whole slab during the burst beat — and the pieces are exactly the regions those cracks enclose. A CPU flood fill (`buildRegionMap`) rasterizes the completed polylines as walls on a fine grid, labels the connected regions, absorbs wall cells and slivers into their largest neighbours, and uploads the result as an `R8` piece-id texture; the display shader's piece-membership test becomes a single texture fetch instead of the Voronoi scan. Shard spin scales inversely with area — big slabs turn lazily, chips tumble. One revealed crack yields two clean halves.
